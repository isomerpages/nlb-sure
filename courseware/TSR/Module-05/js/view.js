...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit125==undefined){
			localStorage.firstVisit125="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray125.split(',');
			pageToLoad=Number(localStorage.crsLoc125);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/Write_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/Write_References.pdf","References");
}

function openTranscript(){
	 
	 openPopUp("resources/Write_Transcript.pdf","Transcript");
}


function openHelp(){
	openPopUp("resources/Write_Help.pdf","Help");
}
