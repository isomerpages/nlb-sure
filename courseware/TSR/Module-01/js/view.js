...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit121==undefined){
			localStorage.firstVisit121="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray121.split(',');
			pageToLoad=Number(localStorage.crsLoc121);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/PR_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/PR_References.pdf","References");
}

function openHelp(){
	openPopUp("resources/PR_Help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/PR_Transcript.pdf","Transcript");
}
