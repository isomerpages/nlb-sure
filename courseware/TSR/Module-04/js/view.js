...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit124==undefined){
			localStorage.firstVisit124="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray124.split(',');
			pageToLoad=Number(localStorage.crsLoc124);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/Analyse_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/Analyse_References.pdf","References");
}

function openHelp(){
	openPopUp("resources/Analyse_Help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/Analyse_Transcript.pdf","Transcript");
}
