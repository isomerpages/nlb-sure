...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit103==undefined){
			localStorage.firstVisit103="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray103.split(',');
			pageToLoad=Number(localStorage.crsLoc103);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/PYDC_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/PYDC_References.pdf","References");
}

function openTranscript(){
	openPopUp("resources/PYDC_Transcript.pdf","Transcript");
}

function openHelp(){
	openPopUp("resources/PYDC_Help.pdf","Help");
}
