...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit102==undefined){
			localStorage.firstVisit102="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray102.split(',');
			pageToLoad=Number(localStorage.crsLoc102);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/EYRO_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/EYRO_References.pdf","References");
}

function openTranscript(){
	openPopUp("resources/EYRO_Transcript.pdf","Transcript");
}

function openHelp(){
	openPopUp("resources/EYRO_Help.pdf","Help");
}
