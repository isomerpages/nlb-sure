...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit101==undefined){
			localStorage.firstVisit101="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray101.split(',');
			pageToLoad=Number(localStorage.crsLoc101);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/SYR_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/SYR_References.pdf","References");
}

function openHelp(){
	openPopUp("resources/SYR_Help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/SYR_Transcript.pdf","Transcript");
}
