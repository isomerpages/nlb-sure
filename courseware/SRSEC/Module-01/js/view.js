...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit111==undefined){
			localStorage.firstVisit111="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray111.split(',');
			pageToLoad=Number(localStorage.crsLoc111);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/Overview_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/Overview_References.pdf","Bibliography & References");
}

function openHelp(){
	openPopUp("resources/Overview_help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/Overview_Transcript.pdf","Transcript");
}
