...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit113==undefined){
			localStorage.firstVisit113="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray113.split(',');
			pageToLoad=Number(localStorage.crsLoc113);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/SRRP2_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/SRRP2_References.pdf","Bibliography & References");
}

function openHelp(){
	openPopUp("resources/SRRP2_help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/SRRP2_Transcript.pdf","Transcript");
}
