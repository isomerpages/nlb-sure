...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit112==undefined){
			localStorage.firstVisit112="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray112.split(',');
			pageToLoad=Number(localStorage.crsLoc112);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/SRRP1_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/SRRP1_References.pdf","Bibliography & References");
}

function openHelp(){
	openPopUp("resources/SRRP1_help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/SRRP1_Transcript.pdf","Transcript");
}
