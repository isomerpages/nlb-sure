...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit114==undefined){
			localStorage.firstVisit114="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray114.split(',');
			pageToLoad=Number(localStorage.crsLoc114);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/SRRP3_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/SRRP3_References.pdf","Bibliography & References");
}

function openHelp(){
	openPopUp("resources/SRRP3_help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/SRRP3_Transcript.pdf","Transcript");
}
