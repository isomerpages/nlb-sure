...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit153==undefined){
			localStorage.firstVisit153="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray153.split(',');
			pageToLoad=Number(localStorage.crsLoc153);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
