...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit151==undefined){
			localStorage.firstVisit151="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray151.split(',');
			pageToLoad=Number(localStorage.crsLoc151);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
