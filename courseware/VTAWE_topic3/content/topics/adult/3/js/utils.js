...
var utils = new function() {
    ...
    this.getScale=function(stage){
        ...
        matches = $(stage).css('-webkit-transform').match(matrixRegex)||$(stage).css('-moz-transform').match(matrixRegex)||$(stage).css('-ms-transform').match(matrixRegex)||$(stage).css('-o-transform').match(matrixRegex)||$(stage).css('transform').match(matrixRegex);
        return matches[1];
    };
};