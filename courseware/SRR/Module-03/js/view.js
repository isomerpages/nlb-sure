...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit83==undefined){
			localStorage.firstVisit83="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray83.split(',');
			pageToLoad=Number(localStorage.crsLoc83);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/HWBS_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/HWBS_References.pdf","Bibliography & References");
}

function openHelp(){
	openPopUp("resources/HWBS_Help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/HWBS_Transcript.pdf","Transcript");
}
