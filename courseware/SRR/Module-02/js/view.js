...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit82==undefined){
			localStorage.firstVisit82="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray82.split(',');
			pageToLoad=Number(localStorage.crsLoc82);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/HBRR_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/HBRR_References.pdf","Bibliography & References");
}

function openHelp(){
	openPopUp("resources/HBRR_Help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/HBRR_Transcript.pdf","Transcript");
}
