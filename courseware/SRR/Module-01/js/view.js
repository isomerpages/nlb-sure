...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit81==undefined){
			localStorage.firstVisit81="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray81.split(',');
			pageToLoad=Number(localStorage.crsLoc81);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openReference(){
	 ...
	 openPopUp("resources/Overview_References.pdf","BIBILIOGRAPHY & REFERENCES");
}
