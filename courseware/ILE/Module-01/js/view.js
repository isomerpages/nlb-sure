...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit91==undefined){
			localStorage.firstVisit91="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray91.split(',');
			pageToLoad=Number(localStorage.crsLoc91);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/WIL_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/WIL_References.pdf","References");
}
...
function openTranscript(){
	openPopUp("resources/WIL_Transcript.pdf","Transcript");
}
