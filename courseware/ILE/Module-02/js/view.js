...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit92==undefined){
			localStorage.firstVisit92="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray92.split(',');
			pageToLoad=Number(localStorage.crsLoc92);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	openPopUp("resources/IMNIP_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/IMNIP_References.pdf","References");
}
...
function openTranscript(){
	openPopUp("resources/IMNIP_Transcript.pdf","Transcript");
}
