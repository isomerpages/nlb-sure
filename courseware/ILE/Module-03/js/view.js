...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit93==undefined){
			localStorage.firstVisit93="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray93.split(',');
			pageToLoad=Number(localStorage.crsLoc93);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/PIL_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/PIL_References.pdf","References");
}
...
function openTranscript(){
	openPopUp("resources/PIL_Transcript.pdf","Transcript");
}
