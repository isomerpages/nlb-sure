...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit173==undefined){
			localStorage.firstVisit173="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray173.split(',');
			pageToLoad=Number(localStorage.crsLoc173);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
