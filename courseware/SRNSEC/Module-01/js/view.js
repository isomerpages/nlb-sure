...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit171==undefined){
			localStorage.firstVisit171="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray171.split(',');
			pageToLoad=Number(localStorage.crsLoc171);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
