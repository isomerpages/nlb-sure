...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit172==undefined){
			localStorage.firstVisit172="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray172.split(',');
			pageToLoad=Number(localStorage.crsLoc172);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
