...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit55==undefined){
			localStorage.firstVisit55="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray55.split(',');
			pageToLoad=Number(localStorage.crsLoc55);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/BRMC_Summary.pdf","Summary");
}
