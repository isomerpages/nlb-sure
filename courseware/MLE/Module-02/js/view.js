...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit52==undefined){
			localStorage.firstVisit52="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray52.split(',');
			pageToLoad=Number(localStorage.crsLoc52);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/MI_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/MCI_References.pdf","References");
}

function openHelp(){
	openPopUp("resources/MCI_Help.pdf","Help");
}

function openTranscript(){
	openPopUp("resources/MCI_Transcript.pdf","Help");
}
