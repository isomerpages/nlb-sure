...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit51==undefined){
			localStorage.firstVisit51="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray51.split(',');
			pageToLoad=Number(localStorage.crsLoc51);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/AM_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/AM_References.pdf","References");
}

function openHelp(){
	openPopUp("resources/AM_Help.pdf","Help");
}


function openTranscript(){
	openPopUp("resources/AM_Transcript.pdf","Help");
}
