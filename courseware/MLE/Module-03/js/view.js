...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit53==undefined){
			localStorage.firstVisit53="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray53.split(',');
			pageToLoad=Number(localStorage.crsLoc53);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/QM_Summary.pdf","Summary");
}
