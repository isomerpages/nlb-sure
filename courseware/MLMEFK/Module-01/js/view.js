...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit61==undefined){
			localStorage.firstVisit61="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray61.split(',');
			pageToLoad=Number(localStorage.crsLoc61);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
