...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit64==undefined){
			localStorage.firstVisit64="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray64.split(',');
			pageToLoad=Number(localStorage.crsLoc64);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
