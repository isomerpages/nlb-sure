...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit63==undefined){
			localStorage.firstVisit63="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray63.split(',');
			pageToLoad=Number(localStorage.crsLoc63);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/TM_Summary.pdf","Summary");
}
