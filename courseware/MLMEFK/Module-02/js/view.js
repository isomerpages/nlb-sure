...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit62==undefined){
			localStorage.firstVisit62="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray62.split(',');
			pageToLoad=Number(localStorage.crsLoc62);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
...
function openSummary(){
	 openPopUp("resources/RM_Summary.pdf","Summary");
}

function openReference(){
	 
	 openPopUp("resources/MLMEFK_References.pdf","References");
}
