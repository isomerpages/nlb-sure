...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit163==undefined){
			localStorage.firstVisit163="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray163.split(',');
			pageToLoad=Number(localStorage.crsLoc163);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
