...
function initShell(){
	...
	if(isLMSReady=="true"){
		...
		if(localStorage.firstVisit162==undefined){
			localStorage.firstVisit162="visited";
		}else{	
			
			visitedArray=localStorage.visitedArray162.split(',');
			pageToLoad=Number(localStorage.crsLoc162);
			if(pageToLoad>0){
				var resume=confirm("Do you want to resume?");
				if(!resume){
					pageToLoad=0;
